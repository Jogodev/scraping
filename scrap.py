...
def get_all_books():
    ...
    for books in link_books:
        ...
        table_book_tax_excl = book_soup.find('table', class_='table table-striped').findNext('td').findNext('td').findNext('td').text
        table_book_tax_incl = book_soup.find('table', class_='table table-striped').findNext('td').findNext('td').findNext('td').findNext('td').text
        table_book_stock = book_soup.find('table', class_='table table-striped').findNext('td').findNext('td').findNext('td').findNext('td').findNext('td').findNext('td').text
        rating = (book_soup.find('p', class_='star-rating').findAll('i'))
        review = book_soup.find('table', class_='table table-striped').findNext('td').findNext('td').findNext('td').findNext('td').findNext('td').findNext('td').findNext('td').text
        ...
        with open("books.csv", "w") as bookfile:
            ...
            bookfile.write(str(link_book) + ' ; ' + str(categ) + ' ; ' + str(title_book) + ' ; ' + str(
                product_description) + ' ; ' + str(table_book_upc) + ' ; ' + str(table_book_tax_excl) + ' ; ' + str(table_book_tax_incl) + ' ; ' + str(table_book_stock) + ' ; ' + str(len(rating)) + ' ; ' + str(review))
